...
private:
    ...
    void leftBoundary(Node *root, vector<int> &result){
        ...
        while(current){
            if(!isLeaf(current)) result.push_back(current->data);
    ...
    void rightBoundary(Node *root, vector<int> &result){
        ...
        while(current){
            if(!isLeaf(current)) dummy.push_back(current->data);
            if(current->right) current = current->right;
            else current  = current->left;
        }
        
        for(int ind = dummy.size()-1; ind>=0; ind--){
            result.push_back(dummy[ind]);
        }
    }
